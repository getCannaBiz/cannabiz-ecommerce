 ...
 * @since    1.0
 */
function wpd_shopping_cart_order_details_metaboxes() {
	add_meta_box(
		'wpdispensary_compounds',
		__( 'Order Details', 'wp-dispensary' ),
		'wpd_shopping_cart_order_details_build',
		'wpd_orders',
		'normal',
		'high'
	);
}
add_action( 'add_meta_boxes', 'wpd_shopping_cart_order_details_metaboxes' );

/**
 * Building the metabox
 */
function wpd_shopping_cart_order_details_build() {
	global $post;

	/** Noncename needed to verify where the data originated */
	echo '<input type="hidden" name="wpd_shopping_cart_order_details_meta_noncename" id="wpd_shopping_cart_order_details_meta_noncename" value="' .
 ...
 */
function wpd_shopping_cart_save_order_details( $post_id, $post ) {
	 ...
	 */
	if (
		! isset( $_POST['wpd_shopping_cart_order_details_meta_noncename' ] ) ||
		! wp_verify_nonce( $_POST['wpd_shopping_cart_order_details_meta_noncename'], plugin_basename( __FILE__ ) )
	) {
		return $post->ID;
	}
	 ...
	 */
	$wpd_shopping_cart_order_meta['wpd_order_status']           = $_POST['wpd_order_status'];
	$wpd_shopping_cart_order_meta['wpd_order_details']          = $_POST['wpd_order_details'];
	$wpd_shopping_cart_order_meta['wpd_order_customer_id']      = $_POST['wpd_order_customer_id'];
	$wpd_shopping_cart_order_meta['wpd_order_customer_address'] = $_POST['wpd_order_customer_address'];

	/** Add values of $wpd_shopping_cart_order_meta as custom fields */

	foreach ( $wpd_shopping_cart_order_meta as $key => $value ) { /** Cycle through the $thccbd_meta array! */
...
add_action( 'save_post', 'wpd_shopping_cart_save_order_details', 1, 2 ); // Save the custom fields.
